...
(function initHelpGuide() {
  ...
  const wrap = overlay.querySelector('#help-lightbox-wrap');
  const prevBtn = overlay.querySelector('#help-lightbox-prev');
  const nextBtn = overlay.querySelector('#help-lightbox-next');
  ...
  function renderGuide(animate = true) {
    ...
    dots.forEach((d, i) => d.classList.toggle('active', i === guideIndex));
  }
  ...
  helpBtn.addEventListener('click', (e) => { e.stopPropagation(); openGuide(); });
  closeBtn.addEventListener('click', (e) => { e.stopPropagation(); closeGuide(); });
  ...
  dots.forEach(dot => {
    dot.addEventListener('click', (e) => {
      e.stopPropagation();
      guideIndex = parseInt(dot.getAttribute('data-idx'));
      renderGuide(true);
    });
  });
